...
class Retry {
      ...
      if (retryCount < 0) {
        ...
        (async () => {
          ...
          .catch(err => {
            ...
            try {
              if (errorLogger !== null) {
                errorLogger(err, retryCount - 1);
              }

              timeoutHandle = setTimeout(() =>
                Retry._run(fn, retryCount - 1, wait, err, errorLogger, resolve, reject), wait);
            }
            catch (err2) {
              reject(err2);
  ...
  static run(fn, wait = -1, retryCount = -1, errorLogger = null) {
    return new Promise(
      (resolve, reject) =>
        Retry._run(fn, retryCount < 0 ? 1000 : retryCount, wait < 0 ? 1 : wait, null, errorLogger, resolve, reject));
  }
}
