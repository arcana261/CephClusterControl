...
class NtpProxy {
      ...
      (await this.hosts())
        .map(async host => {
          return {
            ...
            servers: await this._client.call('ntp', host.hostname, 'ntp.server', [], {timeout: -1})
          };
        }));
  }
  ...
  async makeStep(host = '*') {
    if (!host || host === '*') {
      ...
      const result = await this._client.call('ntp', host, 'ntp.makeStep', [], {timeout: -1});

      if (!result) {
        throw new Error('operation failed: false');
      }
    }
  }
}
