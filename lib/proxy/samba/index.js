...
class SambaProxy {
      ...
      result = {
        [host]: {
          instance: {
            ...
            instance: 'instance',
            success: true,
            data: await this._client.call('samba', host, 'samba.ls', args, {timeout: -1})
          }
        }
      };
    }
   ...
   * @returns {Promise.<SambaShare>}
   */
  async add(share, host = '*') {
    if ((await this.exists(share.name))) {
      throw new Error(`share ${share.name} already exists`);
    }

    if (!host || host === '*') {
      return await this._client.enqueue('samba', 'samba.add', [share], {timeout: -1});
    }
    else {
      return await this._client.call('samba', host, 'samba.add', [share], {timeout: 60000});
    }
  }
  ...
  async del(shareName) {
    ...
    if (host) {
      ...
      result = UnfoldBroadcast.unfold(
        await this._client.broadcastType('samba', 'samba.del', [shareName], {waitForHosts: hosts, timeout: -1})
    ...
    if (host) {
      ...
      result = UnfoldBroadcast.unfold(
        await this._client.broadcastType('samba', 'samba.addUser',
          [shareName, username, acl], {waitForHosts: hosts, timeout: -1})
      ).some(x => x.success && x.data);
    }
    ...
    if (host) {
      ...
      result = UnfoldBroadcast.unfold(
        await this._client.broadcastType('samba', 'samba.delUser',
          [shareName, username], {waitForHosts: hosts, timeout: -1})
      ).some(x => x.success && x.data);
    }
    ...
    if (host) {
      ...
      result = UnfoldBroadcast.unfold(
        await this._client.broadcastType('samba', 'samba.extend',
          [shareName, size], {waitForHosts: hosts, timeout: -1})
      ).some(x => x.success && x.data);
    }
