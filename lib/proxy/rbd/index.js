...
class RbdProxy {
   ...
   */
  hosts({timeout = 0} = {}) {
    return this._client.listHostsForType('rbd', {timeout: timeout});
  }

  /**
   * @returns {Promise.<Array.<String>>}
   */
  ls({pool = 'rbd', id = 'admin', timeout = 0} = {}) {
   ...
   */
  async getMapped({host = '*', id = 'admin', timeout = 0} = {}) {
    let result = null;

    if (!host || host === '*') {
      const hosts =  (await this.hosts({timeout: timeout})).map(x => x.hostname);
      result = await this._client.broadcastType('rbd', 'rbd.getMapped',
        [{id: id}], {timeout: timeout, waitForHosts: hosts});
    }
    else {
      result = {
        [host]: {
          instance: {
            ...
            instance: 'instance',
            success: true,
            data: await this._client.call('rbd', host, 'rbd.getMapped', [{id: id}], {timeout: timeout})
          }
        }
      };
    }
   ...
   */
  async umount({image, host = '*', pool = '*', id = 'admin', force = false, timeout = -1} = {}) {
    const args = [{
      image: image,
      pool: pool,
      id: id,
    ...
    if (!host || host === '*' || (host instanceof Array)) {
      const hosts = (host instanceof Array) ? host : (await this.hosts()).map(x => x.hostname);
      const result = UnfoldBroadcast.unfold(
        await this._client.broadcastType('rbd', 'rbd.umount',
          args, {timeout: timeout, waitForHosts: hosts}));
      ...
      const result = await this._client.call('rbd', host, 'rbd.umount', args, {timeout: timeout});
   ...
   * @returns {Promise.<void>}
   */
  async extend({image, size, host = '*', pool = '*', id = 'admin', timeout = -1} = {}) {
    const name = ImageNameParser.parse(image, pool);
    const mapped =
      ((!(host instanceof Array)) && (!host || host === '*')) ?
        (await this.getMapped({id: 'admin', timeout: timeout}))
          .filter(x => x.image === name.fullName && !x.readOnly)
          .map(x => x.hostname) : ((host instanceof Array) ? host : [host]);
    ...
    if (mapped.length > 0) {
      result = await this._client.call('rbd', mapped[0], 'rbd.extend', args, {timeout: timeout});
    }
    else {
      result = await this._client.enqueue('rbd', 'rbd.extend', args, {timeout: timeout});
    }
