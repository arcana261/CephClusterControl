...
class RbdProxy {
  ...
  mount({image, host = '*', pool = '*', target = '*', readonly = false,
    ...
    if (!host || host === '*') {
      return this._client.enqueue(queue, 'rbd.mount', args, {timeout: -1})
    }
    else {
      return this._client.call(queue, host, 'rbd.mount', args, {timeout: -1});
    }
  }
   ...
   */
  async umount({image, host = '*', pool = '*', id = 'admin'}) {
    const args = [{
      image: image,
      pool: pool,
      id: id
  ...
  /**
   * @returns {Promise.<Array.<{host: String}>>}
   */
