...
/**
 * @typedef {object} _UpdaterAutoSave
 * @property {string} path
 * @property {string} sha
...
class UpdaterProxy {
  ...
  /**
   * @param {string} host
   * @returns {Promise.<_UpdaterAutoSave|null>}
   * @private
   */
  async _readAutoSave(host) {
    const dir = path.join(os.homedir(), '.kaveh-updater.conf.d');
    await MkDir.path(dir);
    const fileName = path.join(dir, host);
   ...
   * @private
   */
  async _writeAutoSave(host, data) {
    const dir = path.join(os.homedir(), '.kaveh-updater.conf.d');
    await MkDir.path(dir);
    const fileName = path.join(dir, host);
  ...
  async _updateHost(report, index, downgrade, cb) {
    ...
    let autoSave = await this._readAutoSave(item.hostname);

    if (item.applicable) {
      let targetPath = null;

      if (autoSave !== null &&
        autoSave.path === report.path &&
        autoSave.sha === report.sha) {

        targetPath = autoSave.generatedPath;
      }
      else {
        ...
        await this._writeAutoSave(item.hostname, autoSave);
      }
