...
/**
 * @typedef {Object.<string,_UpdaterAutoSaveItem>} _UpdaterAutoSave
 */

/**
 * @typedef {object} _UpdaterAutoSaveItem
 * @property {string} path
...
class UpdaterProxy {
  ...
  async _initializeReport(filePath, hosts = null) {
    ...
    return {
      size: size,
      path: filePath,
      sha: await CheckSum.fromFile(filePath),
      version: null,
      target: target,
  ...
  /**
   * @returns {Promise.<_UpdaterAutoSave>}
   * @private
   */
  async _readAutoSave() {
    const fileName = path.join(os.homedir(), '.kaveh-updater.conf');
  ...
  /**
   * @param {_UpdaterAutoSave} data
   * @returns {Promise.<void>}
   * @private
   */
  async _writeAutoSave(data) {
    const fileName = path.join(os.homedir(), '.kaveh-updater.conf');
  ...
  async _updateHost(report, index, downgrade, cb) {
    ...
    let autoSave = await this._readAutoSave();

    if (item.applicable) {
      let targetPath = null;

      if (item.hostname in autoSave &&
        autoSave[item.hostname].path === report.path &&
        autoSave[item.hostname].sha === report.sha) {

        targetPath = autoSave[item.hostname].generatedPath;
      }
      else {
        targetPath = await this._generatePath(item.hostname);

        autoSave[item.hostname] = {
          path: report.path,
          sha: report.sha,
          generatedPath: targetPath
        };

        await this._writeAutoSave(autoSave);
      }
