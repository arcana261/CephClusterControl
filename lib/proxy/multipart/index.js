...
class MultipartProxy {
  ...
  async _transfer(host, filePath, generator, speedReport) {
    ...
    while (!quit) {
      const nextBufferList = await generator(j);
      j = j + 1;

      if (nextBufferList !== null) {
        for (const nextBuffer of nextBufferList) {
          if ((i % QUERY_HASH) === 0) {
            oldHash = await this._queryPartHash(host, filePath, i, QUERY_HASH);
          }
          ...
          if (oldHash[i % QUERY_HASH] !== sha) {
            ...
            await Retry.run(async () => {
              await this._uploadPart(host, filePath, i, part, CheckSum.fromBufferOld(nextBuffer));
  ...
  async _sendFile(source, host, destination, speedReport) {
    ...
    const buffer = new Buffer(204800);
    let last = -1;
    let done = false;
    ...
    try {
      await this._transfer(host, destination, async bucket => {
        if (done) {
          ...
          let [bytesRead] = await fs.read(handle, buffer, 0, 204800, (last + 1) === bucket ? null : bucket * 204800);
          last = bucket;
          ...
          while (bytesRead > 0) {
            let take = Math.min(20480, bytesRead);
            result.push(buffer.slice(offset, offset + take));
            offset += take;
            bytesRead -= take;
          }
