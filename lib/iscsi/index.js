...
class IScsiClient {
   ...
   * @private
   */
  async _deleteLastBackStore(iqnStr, destroyData) {
    const iqn = this._parseIqn(iqnStr);
    ...
    if (iqn.name in luns) {
      ...
      if (lun.sizes.length > 0) {
        const diskName = this._createDiskName(iqn.name, lun.sizes.length - 1);
