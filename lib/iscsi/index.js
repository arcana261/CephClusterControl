...
class IScsiClient {
   ...
   * @private
   */
  async _deleteLastBackStore(name) {
  ...
  async ls() {
    ...
    return await Promise.all(
      (await this._parseTargets())
        .filter(x => x.tpg === 1)
        .map(async target => {
          const auth = await this._getTpgParam(target.stringifiedIqn, 'attribute', 'authentication');
          const userId = await this._getTpgParam(target.stringifiedIqn, 'auth', 'userid');
          const password = await this._getTpgParam(target.stringifiedIqn, 'auth', 'password');
          ...
          if (target.iqn.name in backstores) {
            luns = await this._ensureUsage(backstores[target.iqn.name]);
          }

          return {
            iqn: target.iqn,
            stringifiedIqn: target.stringifiedIqn,
  ...
  async _ensureLuns(iqn, iqnStr) {
    ...
    const actual = (await this._parseLuns()).map(x => x.value);
   ...
   * @returns {Promise.<IScsiTarget>}
   */
  async create({name, host = null, domain = null, image, pool = '*', size} = {}) {
    ...
    await this._addBackStore(name, image, pool, size);
    ...
    try {
      ...
      await this._deleteLastBackStore(name);
      throw err;
    }
