...
class IScsiClient {
  ...
  async _deleteBackStore(iqnStr, destroyData, index, {last = false} = {}) {
    ...
    if (iqn.name in luns) {
      ...
      let lunToDelete = lun.items.filter(x => x.index === index)[0];
      ...
      if (lun.items.length > 0) {
        if (last) {
          lunToDelete = lun.items[lun.items.length - 1];
        }

        await this._deleteBackStoreInternal(iqnStr, iqn, destroyData, lun.image, lun.pool, lunToDelete);
      }
    }
  }
   ...
   * @private
   */
  async _deleteBackStoreInternal(iqnStr, iqn, destroyData, image, pool, lun) {
    const diskName = this._createDiskName(iqn.name, lun.index);
    ...
    if (destroyData) {
      const imagePath = path.join(this.rbd.generateAutoMountPath({image: image, pool: pool}),
        `${diskName}.img`);
   ...
   * @private
   */
  async _addBackStore(name, image, pool, size, {index = -1} = {}) {
    const luns = await this._lsBackStores();
    let lun = null;
    ...
    try {
      ...
      if (index < 0 || lun.items.some(x => x.index === index)) {
        index = lun.items.length < 1 ? 0 : lun.items[lun.items.length - 1].index + 1;
      }

      const diskName = this._createDiskName(name, index);
   ...
   * @private
   */
  async _findTargetByName(name, {usage = true} = {}) {
    ...
    result = await this._convertParsedTargetItem(backstores, result, {usage: usage});
   ...
   * @param {number} size
   * @param {boolean} usage
   * @param {number} lunIndex
   * @returns {Promise.<IScsiTarget>}
   */
  async add({name, host = null, domain = null, image, pool = '*', size, usage = true, lunIndex = -1} = {}) {
    ...
    const backStoreResult = await this._addBackStore(name, image, pool, size, {index: lunIndex});
    ...
    return await this._findTargetByName(name, {usage: usage});
  }
   ...
   * @returns {Promise.<IScsiTarget>}
   */
  async enableAuthentication(name, password, {usage = true} = {}) {
    this._assertPassword(password);
    const target = await this._findTargetByName(name, {usage: false});
   ...
   * @returns {Promise.<IScsiTarget>}
   */
  async disableAuthentication(name, {usage = true} = {}) {
   ...
   * @returns {Promise.<IScsiTarget>}
   */
  async addLun(name, size, {usage = true, index = -1} = {}) {
    const target = await this._findTargetByName(name, {usage: false});
    ...
    await this._addBackStore(name, target.luns.image, target.luns.pool, size, {index: index});
   ...
   * @returns {Promise.<IScsiTarget>}
   */
  async removeLun(name, index, {destroyData = false, usage = true} = {}) {
    const target = await this._findTargetByName(name);
   ...
   * @returns {Promise.<boolean>}
   */
  async del(name, destroyData, {usage = true} = {}) {
    let target = await this._findTargetByName(name, {usage: false});
    let luns = target.luns;

    while (target.luns !== null && target.luns.items.length > 0) {
      for (let i = 0; i < target.luns.items.length; i++) {
        await this._deleteBackStoreInternal(target.stringifiedIqn,
          target.iqn, destroyData, target.luns.image, target.luns.pool, target.luns.items[i]);
      }

      target = await this._findTargetByName(name, {usage: false});
    }

    await Shell.exec('targetcli', `"/iscsi delete ${target.stringifiedIqn}"`);

    if (luns !== null) {
      await Sleep.nap(2000);
  ...
  async rename(name, newName, {usage = true} = {}) {
    ...
    if (target.luns === null || target.luns.items.length < 1) {
    ...
    try {
      ...
      for (let i = 0; i < target.luns.items.length; i++) {
        await fs.rename(
          path.join(directory, `${this._createDiskName(name, target.luns.items[i].index)}.img`),
          path.join(directory, `${this._createDiskName(newName, i)}.img`));
      }

      let newTarget = await this.add({
        name: newName,
        host: target.iqn.host,
        domain: target.iqn.domain,
        image: target.luns.image,
        pool: target.luns.pool,
        size: target.luns.items[0].size,
        usage: false
      });

      for (let i = 1; i < target.luns.items.length; i++) {
        const item = target.luns.items[i];

        newTarget = await this.addLun(newName, item.size, {
          usage: (((i + 1) < target.luns.items.length) || !target.authentication) ? false : usage
        });
      }

      if (target.authentication !== null) {
        newTarget = await this.enableAuthentication(newName, target.authentication.password, {usage: usage});
      }
   ...
   * @returns {Promise.<IScsiTarget>}
   */
  async extend(name, size, {usage = true} = {}) {
    const target = await this._findTargetByName(name, {usage: false});
