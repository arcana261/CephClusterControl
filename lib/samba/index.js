...
class SambaClient {
  ...
  async _stopSamba() {
    ...
    if (distro.ubuntu) {
      await Shell.exec('systemctl stop smbd');
    }
    else if (distro.centos) {
      await Shell.exec('systemctl stop smb.service');
    }
    else {
      throw new Error(`unrecognized os: ${JSON.stringify(distro)}`);
    }
  ...
  async _getSambaStatus() {
    ...
    if (distro.ubuntu) {
      try {
        return await Shell.exec('systemctl status smbd');
      }
      catch (err) {
        return await Shell.exec('systemctl restart smbd');
      }
    }
    else if (distro.centos) {
      try {
        return await Shell.exec('systemctl status smb.service');
      }
      catch (err) {
        return await Shell.exec('systemctl restart smb.service');
      }
    }
    else {
      throw new Error(`unrecognized os: ${JSON.stringify(distro)}`);
  ...
  async ls({info = true} = {}) {
    ...
    for (let share of result) {
      ...
      if (info) {
        const rbdInfo = await this.rbd.info({image: share.image, pool: share.pool, id: share.id});
        share.capacity = rbdInfo.diskSize;
        share.used = rbdInfo.diskUsed;
      }
    }
  ...
  async del(shareName) {
    ...
    if (share) {
      ...
      await this.rbd.umount({image: share.image, pool: share.pool, id: share.id});
