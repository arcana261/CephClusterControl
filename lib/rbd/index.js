...
class RbdClient {
   ...
   */
  async umount({image, pool = '*', id = 'admin'} = {}) {
    const name = ImageNameParser.parse(image, pool);
  ...
  /**
   * @returns {Promise.<{host: String, result: Boolean}>}
   */
  async automount() {
  ...
  /**
   * @returns {Promise.<{location: String, host: String}>}
   */
  async mount({image, pool = '*', target = '*', readonly = false, permanent = false, id = 'admin'} = {}) {
    ...
    if (shouldMap) {
      await this._map({name: name, id: id, readonly: readonly});
    }
