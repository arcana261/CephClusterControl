...
class RbdClient {
  ...
  async _recycleMount({image, _id, _hasMounted, _recycleAsReadOnly} = {}) {
    if (_hasMounted) {
      if (_recycleAsReadOnly) {
        ...
        await this.umount({image: image, id: id});
      }
    }
  }
   ...
   * fileSystem: String|null, rbdId: Number, device: String, image: String,
   * _hasMapped: Boolean, readOnly: Boolean, _hasMounted: Boolean}>}
   */
