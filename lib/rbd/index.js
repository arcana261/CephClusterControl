...
class RbdClient {
  ...
  async info({image, pool = '*', id = 'admin'} = {}) {
    ...
    try {
      mountResult = await this.mount({image: image, pool: pool, readonly: null, id: id, permanent: false});
    }
    catch (err) {
    ...
    const ret = {
      image: name.fullName,
      size: size,
      diskSize: mountResult ? mountResult.diskSize : 0,
      diskUsed: mountResult ? mountResult.diskUsed : 0,
      fileSystem: mountResult ? mountResult.fileSystem : null,
   ...
   * @returns {Promise.<void>}
   */
  async preShutDown({id = 'admin', force = false} = {}) {
    try {
      for (const mapping of (await this._getMappedShort({id: id}))) {
        try {
          const name = ImageNameParser.parse(mapping.image);
          const mountPoint = await this._getMountPoint(mapping.rbdId);
          ...
          log.warn(`Error occured while auto-unmounting "/dev/rbd${mapping.rbdId}" for rbd image "${mapping.image}"`);
          log.warn(ErrorFormatter.format(err));
        }
      }
    }
    catch (err) {
      log.warn(`Error occured while trying to list mountpoints for auto-unmounting process`);
      log.warn(ErrorFormatter.format(err));
    }
  }
