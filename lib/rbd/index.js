...
class RbdClient {
   ...
   * @private
   */
  async _map({image, pool = '*', id = 'admin', readonly = false} = {}) {
    return (await Shell.exec('rbd', 'map', name.fullName, '--id', id, readonly ? '--read-only' : '')).trim();
  }
   ...
   * @private
   */
  async _unmap({image, pool = '*', id = 'admin'} = {}) {
    const name = ImageNameParser.parse(image, pool);
   ...
   * @private
   */
  async _mount({image, target, pool = '*'} = {}) {
    const name = ImageNameParser.parse(image, pool);
  ...
  async mount({image, pool = '*', target = '*', readonly = false, permanent = false, id = 'admin'} = {}) {
    ...
    if (!target || target === '*') {
      target = `/tmp/kaveh-automount/${name.pool}/${name.image}`;
    }
    ...
    if (mapped) {
      ...
      if (readonly !== lastReadOnly) {
        ...
        await this._unmap({image: image, pool: pool, id: id});
      }
      else {
        shouldMap = false;
    ...
    if (shouldMap) {
      await this._map({image: image, pool: pool, id: id, readonly: readonly});
    }

    if (shouldMount) {
      await this._mount({image: image, pool: pool, target: target});
    }
    ...
    if (permanent) {
      ...
      await this._db.delMultiSetItem('rbd:automount', valueToAdd);
      await this._db.putMultiSet('rbd:automount', valueToAdd);
    }
