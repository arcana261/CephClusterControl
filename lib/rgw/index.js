...
class RadosGatewayClient {
  ...
  async enableQuota(username, size) {
    await Shell.exec('radosgw-admin', 'quota', 'set', '--quota-scope=user',
      `--uid="${username}"`, `--max-size=${Math.round(size)}M`);
   ...
   * @returns {Promise.<RadosGatewayUser>}
   */
  async update(username, fullName, email) {
    await Shell.exec('radosgw-admin', 'user', 'modify', `--uid="${username}"`,
      typeCheck('String', fullName) ? `--display-name="${fullName}"` : '',
      typeCheck('String', email) ? `--email="${email}"` : '');
   ...
   * @private
   */
  async _suspend(username, suspend) {
    await Shell.exec('radosgw-admin', 'user', suspend ? 'suspend' : 'enable',
      `--uid="${username}"`);
  ...
  async suspend(username) {
    return this._suspend(username, true);
  }
  ...
  async unsuspend(username) {
    return this._suspend(username, false);
  }
}
